...
def run_train(args):
    ...
    train_instance, word_v, char_v, extra_vl = input2instance(train_input, word_v, char_v, pretrain, extra_vl,
                                                              word_dict, args, "train")
    word_v.freeze()
    char_v.freeze()
    ...
    while True:
        ...
        loss_t1, loss_p_t1, hidden_rep_t, hidden_step1 = decoder(train_action[i][0], hidden_step1, word_rep_t,
                                                                 sent_rep_t, pointer=train_action[i][3],
                                                                 copy_rep_t=None, train=True, state=None, opt=1)
        ...
        loss_t2, hidden_rep_t, hidden_step2 = decoder(train_action_step2, hidden_step2, word_rep_t, sent_rep_t,
                                                      pointer=None, copy_rep_t=copy_rep_t, train=True, state=None,
                                                      opt=2)
        ...
        loss_t3, hidden_rep_t, hidden_step3 = decoder(train_action_step3, hidden_step3, word_rep_t, sent_rep_t,
                                                      pointer=None, copy_rep_t=None, train=True, state=None, opt=3)
        check_loss3 += loss_t3.data.tolist()
        # check_loss3_p += loss_p_t3.data.tolist()

        if check_iter % args.check_per_update == 0:
            print(('epoch %.3f : structure %.5f, relation %.5f, variable %.5f, str_p %.5f' % (
            check_iter * 1.0 / len(train_instance), check_loss1 * 1.0 / args.check_per_update,
            check_loss2 * 1.0 / args.check_per_update, check_loss3 * 1.0 / args.check_per_update,
            check_loss1_p * 1.0 / args.check_per_update)))
            check_loss1 = 0
            check_loss2 = 0
            check_loss3 = 0
        ...
        if check_iter % args.eval_per_update == 0:
            torch.save({"encoder": encoder.state_dict(), "decoder": decoder.state_dict(),
                        "input_representation": input_representation.state_dict()},
                       args.model_path_base + "/model" + str(int(check_iter / args.eval_per_update)))
    # test(args, args.dev_output, dev_instance, dev_comb, actn_v, input_representation, encoder, decoder)


def test(args, output_file, test_instance, test_sep, test_comb, test_input, actn_v, input_representation, encoder,
         decoder):
    ...
    with open(output_file, "w") as w:
        for j, instance in enumerate(test_instance):
            ...
            test_word_rep_t, test_sent_rep_t, test_copy_rep_t, test_hidden_t = encoder(test_input_t, test_comb[j],
                                                                                       test_sep[j], train=False)

            # step 1
            test_hidden_step1 = (
            test_hidden_t[0].view(args.action_n_layer, 1, -1), test_hidden_t[1].view(args.action_n_layer, 1, -1))
            state_step1.reset()
            test_output_step1, test_pointers_step1, test_hidden_rep_step1, test_hidden_step1 = decoder(
                actn_v.toidx("<START>"), test_hidden_step1, test_word_rep_t, test_sent_rep_t, pointer=None,
                copy_rep_t=None, train=False, state=state_step1, opt=1)
            ...
            for k in range(len(test_output_step1)):  # DRS( P1(
                ...
                if actn_v.totok(act1) == "DRS(":
                    ...
                    one_test_output_step2, one_test_hidden_rep_step2, test_hidden_step2, partial_state = decoder(
                        [test_hidden_rep_step1[k], test_pointers_step1[drs_idx]], test_hidden_step2, test_word_rep_t,
                        test_sent_rep_t, pointer=None, copy_rep_t=test_copy_rep_t, train=False, state=state_step2,
                        opt=2)
                ...
                if actn_v.totok(act1) == "SDRS(":
                    ...
                    state_step2.reset_condition(act1, act2)
                    one_test_output_step2, one_test_hidden_rep_step2, test_hidden_step2, partial_state = decoder(
                        [test_hidden_rep_step1[k], -1], test_hidden_step2, test_word_rep_t, test_sent_rep_t,
                        pointer=None, copy_rep_t=test_copy_rep_t, train=False, state=state_step2, opt=2)
                    test_output_step2.append(one_test_output_step2)
                    test_hidden_rep_step2.append(one_test_hidden_rep_step2)
                    # partial_state is to store how many relation it already has
                    state_step2.rel_g, state_step2.d_rel_g = partial_state
            ...
            for act1 in test_output_step1:
                if actn_v.totok(act1) in ["DRS(", "SDRS("]:
                    ...
                    for kk in range(len(test_output_step2[k]) - 1):  # rel( rel( )
                        ...
                        # print "========================="
                        one_test_output_step3, _, test_hidden_step3, partial_state = decoder(
                            test_hidden_rep_step2[k][kk], test_hidden_step3, test_word_rep_t, test_sent_rep_t,
                            pointer=None, copy_rep_t=None, train=False, state=state_step3, opt=3)
...
def run_test(args):
    ...
    test_instance, word_v, char_v, extra_vl = input2instance(test_input, word_v, char_v, pretrain, extra_vl, {}, args,
                                                             "dev")

    test(args, args.test_output, test_instance, test_sep, test_comb, test_input, actn_v, input_representation, encoder,
         decoder)
...
class Demo:
    def __init__(self, args):
        ...
        self.input_representation = sentence_rep(self.word_v.size(), self.char_v.size(), self.pretrain,
                                                 self.extra_vl_size, self.args)
        self.encoder = None
        # if args.encoder == "BILSTM":
        #	from encoder.bilstm import encoder as enc
        # elif args.encoder == "Transformer":
        ...
        self.decoder = dec(self.actn_v.size(), self.args, self.actn_v,
                           [self.cstn_step1, self.cstn_step2, self.cstn_step3])
    ...
    def test(self, docs):
        ...
        for i, t in enumerate(doc_toks):
            if t in ["<s>", "</s>", "|||"]:
                test_sep[-1].append(i)
        test_comb = [get_same_lemma(x) for x in zip(test_input, test_sep)]
        test_instance, _, _, _ = input2instance(test_input, self.word_v, self.char_v, self.pretrain, self.extra_vl, {},
                                                self.args, "dev")
    ...
    def decode(self, test_instance, test_sep, test_comb):
        ...
        for j, instance in enumerate(test_instance):
            ...
            test_word_rep_t, test_sent_rep_t, test_copy_rep_t, test_hidden_t = self.encoder(test_input_t, test_comb[j],
                                                                                            test_sep[j], train=False)

            # step 1
            test_hidden_step1 = (test_hidden_t[0].view(self.args.action_n_layer, 1, -1),
                                 test_hidden_t[1].view(self.args.action_n_layer, 1, -1))
            state_step1.reset()
            test_output_step1, test_pointers_step1, test_hidden_rep_step1, test_hidden_step1 = self.decoder(
                self.actn_v.toidx("<START>"), test_hidden_step1, test_word_rep_t, test_sent_rep_t, pointer=None,
                copy_rep_t=None, train=False, state=state_step1, opt=1)
            # print test_output_step1
            # print [actn_v.totok(x) for x in test_output_step1]
            # print test_hidden_rep_step1
            ...
            test_output_step2 = []
            test_hidden_rep_step2 = []
            test_hidden_step2 = (test_hidden_t[0].view(self.args.action_n_layer, 1, -1),
                                 test_hidden_t[1].view(self.args.action_n_layer, 1, -1))
            ...
            for k in range(len(test_output_step1)):  # DRS( P1(
                ...
                if self.actn_v.totok(act1) == "DRS(":
                    ...
                    state_step2.reset_length(test_copy_rep_t[test_pointers_step1[drs_idx]].size(0))
                    state_step2.reset_condition(act1, act2)
                    one_test_output_step2, one_test_hidden_rep_step2, test_hidden_step2, partial_state = self.decoder(
                        [test_hidden_rep_step1[k], test_pointers_step1[drs_idx]], test_hidden_step2, test_word_rep_t,
                        test_sent_rep_t, pointer=None, copy_rep_t=test_copy_rep_t, train=False, state=state_step2,
                        opt=2)
                    test_output_step2.append(one_test_output_step2)
            ...
            test_output_step3 = []
            test_hidden_step3 = (test_hidden_t[0].view(self.args.action_n_layer, 1, -1),
                                 test_hidden_t[1].view(self.args.action_n_layer, 1, -1))
            state_step3.reset(p_max)
            k = 0
            sdrs_idx = 0
