...
def get_best_match(prod_drs, gold_drs, args, single):
    ...
    mapping_order = smart_mappings + get_mapping_list(candidate_mappings, weight_dict,
                                                      args.restarts - len(smart_fscores), match_clause_dict)
    # Set initial values
    done_mappings = {}
    ...
    for i, map_cur in enumerate(mapping_order):  # number of restarts is number of mappings
        ...
        if tuple(cur_mapping) in done_mappings:
            ...
            while True:
                # get best gain
                (gain, new_mapping, match_clause_dict) = get_best_gain(cur_mapping, candidate_mappings, weight_dict,
                                                                       num_vars, match_clause_dict)
...
def get_mapping_list(candidate_mappings, weight_dict, total_restarts, match_clause_dict):
    ...
    if total_restarts <= 0:  # nothing to do here, we do more smart mappings than restarts anyway
        return []
    random_maps = get_random_set(candidate_mappings, weight_dict, total_restarts,
                                 match_clause_dict)  # get set of random mappings here
    return random_maps
...
def get_random_set(candidate_mappings, weight_dict, map_ceil, match_clause_dict):
    ...
    while len(random_maps) < map_ceil and count_duplicate <= 50:
        ...
        if [cur_mapping, match_num] not in random_maps and len(set([x for x in cur_mapping if x != -1])) == len(
                [x for x in cur_mapping if x != -1]):
...
def add_node_pairs(node_pair1, node_pair2, node_pair3, weight_dict, weight_score, no_match, gold_clause_idx,
                   prod_clause_idx):
    '''Add possible node pairs to weight dict for node pair1'''

    if len([node_pair1, node_pair2, node_pair3]) == len(
            set([node_pair1, node_pair2, node_pair3])):  # node pairs cant be duplicates
        if node_pair1 in weight_dict:
            if (node_pair2, node_pair3) in weight_dict[node_pair1]:
                weight_dict[node_pair1][(node_pair2, node_pair3)].append(
                    [weight_score, no_match, gold_clause_idx, prod_clause_idx])
            else:
                weight_dict[node_pair1][(node_pair2, node_pair3)] = [
                    [weight_score, no_match, gold_clause_idx, prod_clause_idx]]
        else:
...
def add_candidate_mapping_three_vars(prod_clauses, gold_clauses, prod_drs, gold_drs, candidate_mapping, weight_dict, i,
                                     j, add_to_index, add_to_index2):
    ...
    if not args.partial:
        ...
        if allowed_var1 and allowed_var2 and allowed_var3 and normalize(prod_clauses[i][edge_index]) == normalize(
                gold_clauses[j][edge_index]):
            ...
            weight_dict = add_node_pairs(node_pair1, node_pair2, node_pair3, weight_dict, weight_score, no_match,
                                         j + add_to_index, i + add_to_index2)
            weight_dict = add_node_pairs(node_pair2, node_pair1, node_pair3, weight_dict, weight_score, no_match,
                                         j + add_to_index, i + add_to_index2)
            weight_dict = add_node_pairs(node_pair3, node_pair1, node_pair2, weight_dict, weight_score, no_match,
                                         j + add_to_index, i + add_to_index2)
    else:
        # Do partial matching
        ...
        if same_edge_type:
            ...
            if allowed_var1:  # add partial match for single variable 1
                ...
                candidate_mapping, weight_dict = add_single_var_mapping(node1_index_drs1, node1_index_drs2,
                                                                        candidate_mapping, weight_dict, weight_score,
                                                                        no_match, j + add_to_index, i + add_to_index2)
            ...
            if allowed_var1 and allowed_var2:  # add partial match for var 1 and 2
                ...
                candidate_mapping, weight_dict = add_candidate_mapping(candidate_mapping, weight_dict, weight_score,
                                                                       node1_index_drs1,
                                                                       node1_index_drs2, node2_index_drs1,
                                                                       node2_index_drs2, no_match, j + add_to_index,
                                                                       i + add_to_index2)
            ...
            if allowed_var1 and allowed_var3:  # add partial match for var 1 and 3
                ...
                candidate_mapping, weight_dict = add_candidate_mapping(candidate_mapping, weight_dict, weight_score,
                                                                       node1_index_drs1,
                                                                       node1_index_drs2, node3_index_drs1,
                                                                       node3_index_drs2, no_match, j + add_to_index,
                                                                       i + add_to_index2)
            ...
            if allowed_var2 and allowed_var3:  # add partial match for var 2 and 3
                ...
                candidate_mapping, weight_dict = add_candidate_mapping(candidate_mapping, weight_dict, weight_score,
                                                                       node2_index_drs1,
                                                                       node2_index_drs2, node3_index_drs1,
                                                                       node3_index_drs2, no_match, j + add_to_index,
                                                                       i + add_to_index2)
...
def add_candidate_mapping(candidate_mapping, weight_dict, weight_score, node1_index_drs1, node1_index_drs2,
                          node2_index_drs1, node2_index_drs2, no_match, gold_clause_idx, prod_clause_idx):
...
def add_single_var_mapping(node1_index, node2_index, candidate_mapping, weight_dict, weight_score, no_match,
                           gold_clause_idx, prod_clause_idx):
...
def map_two_vars_edges(clauses_prod, clauses_gold, prod_drs, gold_drs, edge_numbers, candidate_mapping, weight_dict,
                       var_index1, var_index2, add_to_index, add_to_index2):
    ...
    for i in range(0, len(clauses_prod)):
        for j in range(0, len(clauses_gold)):
            ...
            if not args.partial:
                ...
                if len(edge_numbers) > 1 and not normalize(clauses_prod[i][edge_numbers[1]]) == normalize(
                        clauses_gold[j][edge_numbers[1]]):
                    allowed_edge = False
                else:
                    allowed_edge = True

                # We do not do partial matching, normal case -- everything should match
                if allowed_var1 and allowed_var2 and allowed_edge and normalize(
                        clauses_prod[i][edge_numbers[0]]) == normalize(clauses_gold[j][edge_numbers[0]]):
                    weight_score = 1  # clause matches for 1
                    no_match = ''  # since we do not partial matching no_match is empty
                    candidate_mapping, weight_dict = add_candidate_mapping(candidate_mapping, weight_dict, weight_score,
                                                                           node1_index_drs1, node1_index_drs2,
                                                                           node2_index_drs1, node2_index_drs2, no_match,
                                                                           j + add_to_index, i + add_to_index2)
            else:
                # Do partial matching
                ...
                if same_edge_type_check(clauses_prod[i][edge_numbers[0]], clauses_gold[j][edge_numbers[0]]):
                    ...
                    if normalize(clauses_prod[i][edge_numbers[0]]) == normalize(clauses_gold[j][edge_numbers[0]]):
                        edge_match = 1 / float(
                            len(clauses_prod[0]))  # edge match results in an increase of 0.25 or 0.33

                if len(edge_numbers) > 1:  # also a constant
                    if normalize(clauses_prod[i][edge_numbers[1]]) == normalize(clauses_gold[j][edge_numbers[1]]):
                        const_match = 1 / float(
                            len(clauses_prod[0]))  # const match results in an increase of 0.25 or 0.33
                ...
                if allowed_var1 and same_edge_type:  # add for single variable match
                    ...
                    candidate_mapping, weight_dict = add_single_var_mapping(int(clauses_prod[i][var_index1][1:]),
                                                                            int(clauses_gold[j][var_index1][1:]),
                                                                            candidate_mapping, weight_dict,
                                                                            weight_score, no_match, j + add_to_index,
                                                                            i + add_to_index2)
                ...
                if allowed_var2 and same_edge_type:  # add for single variable match
                    ...
                    weight_score = 1 / float(len(clauses_prod[0])) + edge_match + const_match
                    candidate_mapping, weight_dict = add_single_var_mapping(int(clauses_prod[i][var_index2][1:]),
                                                                            int(clauses_gold[j][var_index2][1:]),
                                                                            candidate_mapping, weight_dict,
                                                                            weight_score, no_match, j + add_to_index,
                                                                            i + add_to_index2)
...
def compute_pool(prod_drs, gold_drs):
    ...
    edge_nums = [1]
    candidate_mapping, weight_dict = map_two_vars_edges(prod_drs.op_two_vars, gold_drs.op_two_vars, prod_drs, gold_drs,
                                                        edge_nums, candidate_mapping, weight_dict, var_spot1, var_spot2,
                                                        count_clauses_prod, count_clauses_gold)

    # Clause looks like (var1, OPR, var2, "item")
    candidate_mapping, weight_dict = map_two_vars_edges(prod_drs.op_two_vars_abs, gold_drs.op_two_vars_abs, prod_drs,
                                                        gold_drs, [1, 3], candidate_mapping, weight_dict, 0, 2,
                                                        len(gold_drs.op_two_vars), len(prod_drs.op_two_vars))
    ...
    for i in range(0, len(prod_drs.op_three_vars)):
        for j in range(0, len(gold_drs.op_three_vars)):
            candidate_mapping, weight_dict = add_candidate_mapping_three_vars(prod_drs.op_three_vars,
                                                                              gold_drs.op_three_vars, prod_drs,
                                                                              gold_drs, candidate_mapping, weight_dict,
                                                                              i, j, len(gold_drs.op_two_vars) + len(
                    gold_drs.op_two_vars_abs), len(prod_drs.op_two_vars) + len(prod_drs.op_two_vars_abs))

    # Clause looks like (var1, Role, var2, "item")
    candidate_mapping, weight_dict = map_two_vars_edges(prod_drs.roles_abs, gold_drs.roles_abs, prod_drs, gold_drs,
                                                        [1, 3], candidate_mapping, weight_dict, 0, 2,
                                                        len(gold_drs.op_two_vars) + len(gold_drs.op_two_vars_abs) + len(
                                                            gold_drs.op_three_vars),
                                                        len(prod_drs.op_two_vars) + len(prod_drs.op_two_vars_abs) + len(
                                                            prod_drs.op_three_vars))

    # Clause looks like (var1, Role, var2, var3)
    for i in range(0, len(prod_drs.roles)):
        for j in range(0, len(gold_drs.roles)):
            candidate_mapping, weight_dict = add_candidate_mapping_three_vars(prod_drs.roles, gold_drs.roles, prod_drs,
                                                                              gold_drs, candidate_mapping, weight_dict,
                                                                              i, j, len(gold_drs.op_two_vars) + len(
                    gold_drs.op_two_vars_abs) + len(gold_drs.roles_abs) + len(gold_drs.op_three_vars),
                                                                              len(prod_drs.op_two_vars) + len(
                                                                                  prod_drs.op_two_vars_abs) + len(
                                                                                  prod_drs.roles_abs) + len(
                                                                                  prod_drs.op_three_vars))

    # Clause looks like (var1, concept, "sns", var2)
    candidate_mapping, weight_dict = map_two_vars_edges(prod_drs.concepts, gold_drs.concepts, prod_drs, gold_drs,
                                                        [1, 2], candidate_mapping, weight_dict, 0, 3,
                                                        len(gold_drs.op_two_vars) + len(gold_drs.op_two_vars_abs) + len(
                                                            gold_drs.roles_abs) + len(gold_drs.op_three_vars) + len(
                                                            gold_drs.roles),
                                                        len(prod_drs.op_two_vars) + len(prod_drs.op_two_vars_abs) + len(
                                                            prod_drs.roles_abs) + len(prod_drs.op_three_vars) + len(
                                                            prod_drs.roles))
...
def get_matches(current_node_pair, key, drs_match):
    ...
    for item in drs_match.weight_dict[current_node_pair][
        key]:  # item looks like this [score, tuple of node pairs, gold_clause_idx, prod_clause_idx] e.g. [0.75, ((2,4), (3,5)), 4, 6]
...
def compute_match_for_key(key, current_node_pair, drs_match):
    ...
    if key == -1:
        ...
        if sorted_tuple in drs_match.matched_clause_keys:  # already matched, don't do again - e.g. if we matched (0,0),(1,1),(2,2) we don't want to also match (1,1), (0,0),(2,2), because they match the same clause
            ...
            if drs_match.mapping[key[0][0]] == key[0][1] and drs_match.mapping[key[1][0]] == key[1][
                1]:  # both nodes also match
                drs_match = get_matches(current_node_pair, key, drs_match)
                drs_match.matched_clause_keys[sorted_tuple] = 1
    ...
    # again in the same box
        ...
        for tup_idx, tup_set in enumerate(clause_list):
            ...
            for idx, spl_tup in enumerate(tup_set):
                if spl_tup[1] == 'REF':
                    if not var_occurs(tup_set, spl_tup[2], spl_tup[0],
                                      idx):  # only add if the variable does not occur afterwards
...
def is_role(string):
    ...
    return string[0].isupper() and any(x.islower() for x in string[1:]) and all(
        x.islower() or x.isupper() or x == '-' for x in string)
...
def create_tab_list(print_rows, print_item, joiner):
    ...
    if print_rows:
        ...
        for idx, row in enumerate(print_rows):  # print rows here, adjusted for column width
            return_rows.append(
                joiner.join(word.decode('utf-8').ljust(col_widths[col_idx]) for col_idx, word in enumerate(row)))
    return return_rows
...
def get_matching_clauses(arg_list):
    ...
    if single and (args.max_clauses > 0 and (
            (prod_drs.total_clauses > args.max_clauses) or (gold_drs.total_clauses > args.max_clauses))):
        ...
        (best_mapping, best_match_num, found_idx, smart_fscores, clause_pairs) = get_best_match(prod_drs, gold_drs,
                                                                                                args, single)
        ...
        (precision, recall, best_f_score) = compute_f(best_match_num, prod_drs.total_clauses, gold_drs.total_clauses,
                                                      args.significant, False)

        print_match, print_no_match, match_division = get_mappings(clause_pairs, prod_drs, gold_drs, precision, recall,
                                                                   args.significant)
        ...
        if args.prin and (args.ms or single):
            ...
            uniq_rewrite = []
            [uniq_rewrite.append(item) for item in rewrite_list if
             item not in uniq_rewrite]  # only unique transformations, but keep order
        ...
        if args.ms and not single:
            print_results([[best_match_num, prod_drs.total_clauses, gold_drs.total_clauses, smart_fscores, found_idx,
                            match_division, prod_clause_division, gold_clause_division, len(prod_drs.var_map)]],
                          start_time, single, args)

        return [best_match_num, prod_drs.total_clauses, gold_drs.total_clauses, smart_fscores, found_idx,
                match_division, prod_clause_division, gold_clause_division, len(prod_drs.var_map)]
...
def print_results(res_list, start_time, single, args):
    ...
    found_idx = round(float(sum([x[4] for x in res_list if x])) / float(len([x[4] for x in res_list if x])),
                      args.significant)
    ...
    prec_op, rec_op, f_op = compute_f(sum([x[0] for x in match_division]), sum([x[0] for x in prod_division]),
                                      sum([x[0] for x in gold_division]), args.significant, False)
    prec_role, rec_role, f_role = compute_f(sum([x[1] for x in match_division]), sum([x[1] for x in prod_division]),
                                            sum([x[1] for x in gold_division]), args.significant, False)
    prec_conc, rec_conc, f_conc = compute_f(sum([x[2] for x in match_division]), sum([x[2] for x in prod_division]),
                                            sum([x[2] for x in gold_division]), args.significant, False)
    ...
    if not res_list:
        ...
        if args.prin:
            ...
            if args.smart == 'conc':
                smart_conc = compute_f(sum([y[0] for y in [x[3] for x in res_list]]), total_test_num, total_gold_num,
                                       args.significant, True)
...
def check_input(clauses_prod_list, original_prod, original_gold, clauses_gold_list, baseline, f1, max_clauses, single):
    ...
    if baseline:  # if we try a baseline DRS, we have to fill a list of this baseline
        ...
        raise ValueError(
            "Number of DRSs not equal, {0} vs {1}, exiting...".format(len(clauses_prod_list), len(clauses_gold_list)))
    ...
    if max_clauses > 0 and not single:
        print('Skipping {0} DRSs due to their length exceeding {1} (--max_clauses)\n'.format(len(
            [x for x, y in zip(clauses_gold_list, clauses_prod_list) if len(x) > max_clauses or len(y) > max_clauses]),
                                                                                             max_clauses))
...
class DRS:
    ...
    op_boxes = ['NOT', 'POS', 'NEC', 'IMP', 'DIS', 'PRP', 'DRS', 'ANSWER', 'PARALLEL', 'CONTINUATION', 'CONTRAST',
    ...
    def add_concept_clauses(self, cur_clause, val0, idx, en_sense_dict):
        ...
        if not between_quotes(cur_clause[2]):
            raise ValueError(
                "Incorrectly format clause: {0}\nThird item of concept clause should be a sense and should always be between quotes".format(
                    cur_clause))
        elif between_quotes(cur_clause[3]):
            raise ValueError(
                "Incorrectly format clause: {0}\n Fourth item of concept clause should be a variable and not between quotes".format(
                    cur_clause))
        else:
    ...
    def add_role_clauses(self, cur_clause, val0, idx):
        ...
        if between_quotes(cur_clause[2]):
            raise ValueError(
                "Incorrectly format clause: {0}\nThird item of role clause should be a variable and not between quotes".format(
                    cur_clause))
        else:
            ...
            if between_quotes(cur_clause[3]):
                self.add_if_not_exists(self.roles_abs, self.roles_abs_idx, (val0, cur_clause[1], val2, cur_clause[3]),
                                       idx)
    ...
    def add_operator_clauses(self, val0, cur_clause, idx):
        ...
        if between_quotes(cur_clause[3]):
            self.add_if_not_exists(self.op_two_vars_abs, self.op_two_vars_abs_idx,
                                   (val0, cur_clause[1], val2, cur_clause[3]), idx)
    ...
    def get_specific_clauses(self, clause_list, en_sense_dict):
        ...
        for idx, cur_clause in enumerate(clause_list):
            ...
            if len(cur_clause) == 3:
                if between_quotes(cur_clause[2]) or between_quotes(cur_clause[2]) or not cur_clause[0].islower() or not \
                cur_clause[2].islower():
                    raise ValueError(
                        "Incorrectly format clause: {0}\nClauses of length three should always contain two variables".format(
                            cur_clause))
                else:
...
def main(arguments):
    ...
    original_prod, clauses_prod_list = check_input(clauses_prod_list, original_prod, original_gold, clauses_gold_list,
