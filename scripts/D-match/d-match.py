...
def get_best_match(attribute1, relation1, attribute2, relation2,
                   prefix1, prefix2, var_count, var_types1, var_types2, var_map_prod, var_map_gold, prin, single,
                   significant, memory_limit, counter):
    """
    ...
    (candidate_mappings, weight_dict) = compute_pool(attribute1, relation1, attribute2, relation2,
                                                     prefix1, prefix2, var_count, var_types1, var_types2, var_map_prod,
                                                     var_map_gold)
    ...
    smart_mappings = get_smart_mappings(candidate_mappings, attribute1, attribute2, relation1, relation2, weight_dict,
                                        args.restarts, args.smart, match_triple_dict)  # get smart mappings
    num_smart = len(smart_mappings)
    smart_fscores = [0] * num_smart

    mapping_list = get_mapping_list(candidate_mappings, weight_dict, args.restarts - num_smart, False,
                                    match_triple_dict)  # get random mappings, or best mapping + random mappings in order depending on args.best_map
    ...
    for i, map_cur in enumerate(mapping_order):  # number of restarts is number of mappings
        ...
        while True:
            # get best gain
            (gain, new_mapping) = get_best_gain(cur_mapping, candidate_mappings, weight_dict, len(var_map_gold),
                                                match_num, match_triple_dict)
            if veryVerbose:
        ...
        if best_match_num == total_triples and i > num_smart - 1:  # if we have matched as much we can (precision 1.0), we might as well stop instead of doing all other restarts - but always do smart mappings
            (precision, recall, f_score) = compute_f(best_match_num, test_triple_num, gold_triple_num, significant,
                                                     False)
    ...
    avg_f = round(float(sum(all_fscores)) / float(len(all_fscores)),
                  args.significant)  # average F-score over all restarts
...
def get_smart_mappings(candidate_mappings, attribute1, attribute2, relation1, relation2, weight_dict, restarts, smart,
                       match_triple_dict):
    ...
    if smart == 'no':
        ...
        smart_conc = smart_concept_mapping(candidate_mappings, relation1, relation2)
        smart_mappings_all = [[smart_attr, compute_match(smart_attr, weight_dict, match_triple_dict)],
                              [smart_order, compute_match(smart_order, weight_dict, match_triple_dict)],
                              [smart_init, compute_match(smart_init, weight_dict, match_triple_dict)],
                              [smart_conc, compute_match(smart_conc, weight_dict, match_triple_dict)]]
...
def get_mapping_list(candidate_mappings, weight_dict, total_restarts, select_best, match_triple_dict):
    ...
    random_maps = get_random_set(candidate_mappings, weight_dict, map_ceil,
                                 match_triple_dict)  # get set of random mappings here

    if select_best:  # we only want the best random mapping
        sorted_maps = sorted(random_maps, key=lambda x: x[1],
                             reverse=True)  # sort mappings by number of initial matches
        if sorted_maps:
...
def get_random_set(candidate_mappings, weight_dict, map_ceil, match_triple_dict):
    ...
    while len(
            random_maps) < map_ceil and count_duplicate <= 50:  # only do random mappings we haven't done before, but if we have 100 duplicates in a row, we are probably done with all mappings
    ...
    for i in range(0, len(attribute1)):
        for j in range(0, len(attribute2)):
            ...
            if normalize(attribute1[i][0]) == normalize(attribute2[j][0]) \
                ...
                if var_types1[attribute1[i][1]] == var_types2[
                    attribute2[j][1]]:  # does the variable type match? -- should always match I think
    ...
    for i in range(0, len(relation1)):
        for j in range(0, len(relation2)):
            ...
            if normalize(relation1[i][0]) == normalize(relation2[j][0]):
                if var_types1[relation1[i][1]] == var_types2[relation2[j][1]] and var_types1[relation1[i][2]] == \
                        var_types2[relation2[j][2]]:  # check if nodes are even allowed to match, based on their type
                    ...
                    for rel2 in relation2:
                        if int(rel2[1][1:]) == node_index and not matched:  # relation exists for this node index
                            ...
                            if concept1 == concept2:  # both should match for a smart mapping match
                                if int(var4.replace('b', '')) in candidate_mapping[
                                    int(rel[2][1:])]:  # second variable also needs to match (possibly)
    ...
    for i, m in enumerate(mapping):
        ...
        for key in weight_dict[current_node_pair]:
            ...
            #     weight_dict[node_pair2][node_pair1] for a relation
                ...
                continue
            elif mapping[key[0]] == key[
                1]:  # key is also a node pair. If we have e.g. node_pair(1,5), we check if indeed mapping[1] is 5, meaning that in the current mapping 1 maps to 5
                match_num += weight_dict[current_node_pair][key]
                if veryVerbose:
    ...
    for idx1, triples in enumerate(triple_list):
        ...
        for equ in equ_triples:  # check if we have to replace ARG1/ARG2 in some triples
            for idx2, t in enumerate(triples):
                if t[0] == equ and (
                        t[1] == 'ARG1' or t[1] == 'ARG2'):  # found equ variable again, now with ARG1 or ARG2 edge
...
def check_triple_match(new_prod_triples, new_gold_triples, attributes1, relation1, attributes2, relation2, var_map_prod,
                       var_map_gold):
    ...
    new_gold_triples_lower = [(x.lower(), y.lower(), z.lower()) for (x, y, z) in
                              new_gold_triples]  # comparison has to be in lower case
    ...
    for idx, p in enumerate(new_prod_triples):
        ...
        for idx2, gold_triple in enumerate(new_gold_triples_lower):
            ...
            if compare_triple == gold_triple:
                match.append([" ".join([str(x) for x in (prod_var1, prod_arg, prod_var2)]), " ".join(
                    [str(x) for x in (gold_var1, gold_arg, gold_var2)])])  # add matched triple for both languages
    ...
    for idx, gold_triple in enumerate(
            new_gold_triples):  # add gold non-matched printed to no_match so that they are also printed
        ...
        if gold_triple not in matched_gold:  # was never matched
            if added < len(no_match):
                no_match[added][1] = " ".join(
                    [str(x) for x in (gold_var1, gold_arg, gold_var2)])  # switch order back to original order
                added += 1
            else:
...
def print_mapping(best_mapping, attributes1, relation1, attributes2, relation2, var_map_prod, var_map_gold, precision,
                  recall, prin, prod_file, gold_file):
    '''Print the matching and non-matching triples in our final mapping - either to screen or to a file'''

    new_mapping = {}

    for var1, var2 in enumerate(
            best_mapping):  # create new shared mapping: both a1 and b3 match to c3, for example. Don't add mapping for -1, means no mapping found
    ...
    match, no_match = check_triple_match(new_prod_triples, new_gold_triples, attributes1, relation1, attributes2,
                                         relation2, var_map_prod, var_map_gold)
    ...
    if prec != precision or rec != recall:
        raise ValueError(
            "Error in calculating precision and recall when doing triple match - different values in outcome")
...
def get_matching_triples(arg_list):
    ...
    attributes1, relation1, var_count1, var_types1, var_map_prod = get_specific_triples(prod_t,
                                                                                        prefix1)  # prefixes 'a' and 'b' are used to create standardized variable-names
    ...
    if single and (args.max_triples > 0 and (
            len(attributes1) + len(relation1) > args.max_triples or len(attributes2) + len(
            relation2) > args.max_triples)):
    ...
    (best_mapping, best_match_num, found_idx, restarts_done, avg_f, smart_fscores) = get_best_match(attributes1,
                                                                                                    relation1,
                                                                                                    attributes2,
                                                                                                    relation2,
                                                                                                    prefix1, prefix2,
    ...
    if prin and (args.ms or single) and not args.no_mapping:
        ...
        print_mapping(best_mapping, attributes1, relation1, attributes2, relation2, var_map_prod, var_map_gold,
                      precision, recall, prin, prod_file, gold_file)
    if args.ms and not single:  # for single DRG we print results later on anyway
        print_results(
            [[best_match_num, test_triple_num, gold_triple_num, smart_fscores, avg_f, restarts_done, found_idx]], False,
            start_time, prin, ms, smart, restarts, significant, max_triples, single)
...
def print_results(res_list, no_print, start_time, prin, ms, smart, restarts, significant, max_triples, single):
    ...
    restarts_done = round(float(sum([x[5] for x in res_list if x])) / float(len([x[5] for x in res_list if x])),
                          significant)
    found_idx = round(float(sum([x[6] for x in res_list if x])) / float(len([x[6] for x in res_list if x])),
                      significant)
    ...
    if not res_list:
        ...
        if prin:  # print specific output here
            print('\n## Detailed F-scores ##\n')
            if smart != 'no':
                smart_first = compute_f(sum([y[0] for y in [x[3] for x in res_list]]), total_test_num, total_gold_num,
                                        significant, True)

            if smart == 'all':
                smart_att = compute_f(sum([y[0] for y in [x[3] for x in res_list]]), total_test_num, total_gold_num,
                                      significant, True)
                smart_order = compute_f(sum([y[1] for y in [x[3] for x in res_list]]), total_test_num, total_gold_num,
                                        significant, True)
                smart_init = compute_f(sum([y[2] for y in [x[3] for x in res_list]]), total_test_num, total_gold_num,
                                       significant, True)
                smart_conc = compute_f(sum([y[3] for y in [x[3] for x in res_list]]), total_test_num, total_gold_num,
                                       significant, True)
...
def main(arguments):
    ...
    if args.max_triples > 0 and not single:  # print number of DRGs we skip due to the -max_triples parameter
        print('Skipping {0} DRGs due to their length exceeding {1} (-max_triples)\n'.format(len(
            [x for x, y in zip(triples_gold_list, triples_prod_list) if
             (len(x) > args.max_triples or len(y) > args.max_triples)]), args.max_triples))
    ...
    for idx in range(args.runs):  # for experiments we want to more runs so we can average later
        ...
        for prod_t, gold_t in zip(triples_prod_list, triples_gold_list):
            counter += 1
            arg_list.append(
                [prod_t, gold_t, args, prefix1, prefix2, args.significant, args.prin, args.f1, args.f2, single, args.ms,
                 args.smart, args.restarts, args.max_triples, args.mem_limit, counter])
        ...
        if all_results:
            if all_results[0]:
                res.append(print_results(all_results, no_print, start, args.prin, args.ms, args.smart, args.restarts,
                                         args.significant, args.max_triples, single))
            else:
...
def build_arg_parser():
    ...
    parser.add_argument('-m', '--max_triples', type=int, default=0,
                        help='Maximum number of triples for DRG (default 0 means no limit)')
    ...
    parser.add_argument('-s', '--smart', default='all', action='store',
                        choices=['no', 'all', 'order', 'freq', 'conc', 'att', 'init'],
                        help='What kind of smart mapping do we use (default all)')
    parser.add_argument('-prin', action='store_true',
                        help='Print very specific output - matching and non-matching triples and specific F-scores for smart mappings')
    parser.add_argument('-sense', default='normal', action='store', choices=['normal', 'wrong', 'ignore', 'base', ],
                        help='How do we evaluate sense - wrong means sense is always wrong, for "ignore" sense is always correct, "base" means always adding -01 for prod file. Default is normal.')
    ...
    parser.add_argument('-mem', '--mem_limit', type=int, default=500,
                        help='Memory limit in MBs (default 500). Note that this is per parallel thread! If you use -par 4, each thread gets 500 MB with default settings.')
    parser.add_argument('-b', '--baseline', action='store_true', default=False,
                        help="Helps in deciding a good baseline DRG. If added, prod-file must be a single DRG, gold file a number of DRGs to compare to (default false)")
    ...
    if args.ms and args.runs > 1:
        raise NotImplementedError(
            "Not implemented to average over individual scores, only use -ms when doing a single run")

    if args.restarts < 1:
        raise ValueError('Number of restarts must be larger than 0')
    elif args.smart == 'all' and args.restarts < 4:
        raise ValueError("All smart mappings is already 4 restarts, therefore -r should at least be 4, not {0}".format(
            args.restarts))
