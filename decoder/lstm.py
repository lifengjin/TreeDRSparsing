...
class decoder(nn.Module):
    ...
    def forward_1(self, input, hidden, word_rep_t, sent_rep_t, pointer, train, state):
        ...
        if train:
            ...
            hidden_t = (
            hidden[0].view(self.args.action_n_layer, 1, -1), hidden[1].view(self.args.action_n_layer, 1, -1))
            action_t = self.embeds(input_t).unsqueeze(1)
            ...
            while True:
                ...
                if self.actn_v.totok(idx) == "DRS(":
                    p_attn_scores_t = torch.bmm(self.pointer_head(output).transpose(0, 1), sent_rep_t.transpose(1, 2))[
                        0]
            ...
            while not all_terminal:
                ...
                while b < len(beamMatrix[-1]):
                    ...
                    feat_hiddens_t = self.feat_tanh(
                        self.feat(torch.cat((attn_hiddens_t, beam.action_t.view(output.size(0), -1)), 1)))
                    global_scores_t = self.out(feat_hiddens_t)
            ...
            for struct, rels, p in input:
                ...
                if p != -1:
                    copy_scores_t = torch.bmm(self.copy_head(output).transpose(0, 1),
                                              copy_rep_t[p].transpose(0, 1).unsqueeze(0)).view(output.size(0), -1)
            ...
            while not all_terminal:
                ...
                while b < len(beamMatrix[-1]):
                    ...
                    if self.args.const:
                        ...
                        if self.cstn2.isterminal(beam.state) or len(beamMatrix) > (
                                self.args.rel_l + self.args.d_rel_l) * 2:
                            tmp.append([beam.score, -1, b])
                            b += 1
                            continue
                    all_terminal = False
                    ...
                    copy_scores_t = torch.bmm(self.copy_matrix(output).transpose(0, 1),
                                              copy_rep_t.transpose(0, 1).unsqueeze(0)).view(output.size(0), -1)
                    # copy_scores_t = torch.bmm(torch.bmm(output.transpose(0,1), self.copy_matrix), encoder_rep_t.transpose(0,1).unsqueeze(0)).view(output.size(0), -1)
                    # print copy_scores_t
...
class decoder_soft(nn.Module):
    ...
    def forward_1(self, input, hidden, word_rep_t, sent_rep_t, pointer, train, state):
        ...
        if train:
            ...
            feat_hiddens_t = self.feat_tanh(self.feat(
                torch.cat((w_attn_hiddens_t, s_attn_hiddens_t, p_attn_hiddens_t, action_t.view(output.size(0), -1)),
                          1)))
            global_scores_t = self.out(feat_hiddens_t)
